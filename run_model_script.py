...
files_To_Run = {
    'PIPELINE_CONFIG':os.path.join('fine_tuned_model' ,'pipeline.config'),
...
print(files_To_Run['PIPELINE_CONFIG'])
...
ckpt.restore(os.path.join(paths_To_Run['CHECKPOINT_PATH'], 'ckpt-7')).expect_partial()
...
def process_imagen(image_data):
    ...
    print("hola process image")
    print(nparr)
    img_np = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
    print("hola img_np image")
    print(img_np)
...
def image_processor(barrier, image_queue, detections_queue):
    ...
    while True:
        try:
            ...
            data = {'detection_classes': detections['detection_classes'],'detection_scores':detections['detection_scores'],'ruta_destino': ruta_destino,'image':image_np_with_detections}
...
def detections_processor(detections_queue,detections_barrier, score, target ):
    ...
    print("hola detecction posprocess")
    while True:
        try:
            # Envía mensajes al servidor
            ...
            i=0
            keep_looking=True
            while detection_scores[i]>=score and keep_looking:
...
for _ in range(max_workers):
    processor_thread = threading.Thread(target=detections_processor, args=(detections_queue,detections_barrier, score, target ))
    processor_thread.start()
    processor_threads.append(processor_thread)

print("hola antes socket")
detections_barrier.wait()
